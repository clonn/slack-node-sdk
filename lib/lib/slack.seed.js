...
Slack = (function() {
  ...
  Slack.prototype.setWebhook = function(url, options) {
    this.webhookUrl = url;
  ...
  Slack.prototype.webhook = function(options, callback) {
    var emoji, payload;
    emoji = this.detectEmoji(options.icon_emoji || this.webhookOptions.response_type);
    payload = {
      response_type: options.response_type || this.webhookOptions.response_type || 'ephemeral',
      channel: options.channel || this.webhookOptions.channel,
      text: options.text,
      username: options.username || this.webhookOptions.username,
      attachments: options.attachments || this.webhookOptions.attachments,
      link_names: options.link_names || this.webhookOptions.link_names || 0
    };
